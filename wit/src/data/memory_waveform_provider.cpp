...
MemoryWaveformProvider::MemoryWaveformProvider(std::vector<uint8_t>&& pcm_data,	 uint64_t sample_count,
...
uint64_t MemoryWaveformProvider::Read(uint64_t sample_offset, uint64_t sample_count, float*	out_left, float* out_right) noexcept {
	...
	/// HACK: 1-channel is out of scope during the beta phase
	if (out_right == nullptr) return 0;
